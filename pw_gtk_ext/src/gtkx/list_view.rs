...
#[derive(PWO, WClone)]
pub struct ListView(Rc<ListViewCore>);
...
impl ListView {
    ...
    pub fn append_row(&self, row: &[glib::Value]) {
        self.0.list_store.append_row(row);
    }

    pub fn insert_row(&self, index: i32, row: &[glib::Value]) {
        self.0.list_store.insert_row(index, row);
    }

    pub fn prepend_row(&self, row: &[glib::Value]) {
        self.0.list_store.prepend_row(row);
    }

    pub fn repopulate_with(&self, rows: &[Vec<glib::Value>]) {
        self.0.list_store.repopulate_with(rows)
    }

    pub fn update_with(&self, rows: &[Vec<glib::Value>]) {
        self.0.list_store.update_with(rows)
    }
}
...
impl ListViewBuilder {
    ...
    pub fn build<L: ListViewSpec>(self, list_view_spec: &L) -> ListView {
        ...
        let blv = ListView(Rc::new(ListViewCore {
            view,
            list_store,
